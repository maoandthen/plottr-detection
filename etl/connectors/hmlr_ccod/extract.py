...
DATASET_NAME = "ccod"
RAW_DIR = Path(os.getenv("DATA_DIR", "/Volumes/PlottrData")) / "raw" / "ccod"
...
def _resolve_full_file() -> tuple[str, str]:
    """Find the latest CCOD_FULL_*.zip resource and return (filename, download_url)."""
    ...
    full_files = [r for r in resources if r["file_name"].startswith("CCOD_FULL_")]
    if not full_files:
        raise RuntimeError(f"No CCOD_FULL_*.zip in resources: {resources}")
