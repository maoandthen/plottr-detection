"""Transform raw OCOD CSV rows into (title_dict, [company_dict], [owned_by_dict]) tuples.

OCOD is the overseas-owner equivalent of CCOD and additionally includes
Country Incorporated columns for each proprietor.
"""
...
def transform(csv_path: Path) -> Generator[tuple[dict, list[dict], list[dict]], None, None]:
    """
    Yields (title_dict, company_dicts, owned_by_dicts) for each valid OCOD row.
    Skips rows where title_number is blank.
    company_dicts include country_incorporated from the OCOD-specific column.
    """
    with open(csv_path, newline="", encoding="utf-8-sig") as fh:
        ...
        for row in reader:
            ...
            for n in range(1, 5):
                ...
                country_col = f"Country Incorporated ({n})"
                ...
                country_incorporated = (row.get(country_col) or "").strip() or None
